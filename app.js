...
// ============================================
// >>> GLIEDERUNGSPUNKT 14: ADMIN BEREICH (COACHES)
// ============================================
...
async function checkIfCoach() {
  ...
  isCoach = data.role === "admin" || data.role === "coach";
  if (isCoach) {
...
function initializeAdminArea() {
  // Tabs
  document.querySelectorAll("#adminArea .tab-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll("#adminArea .tab-btn").forEach(b => b.classList.remove("active"));
  ...
  // Suche
  document.getElementById("searchUsers")?.addEventListener("input", debounce(loadAdminUsers, 300));
...
async function loadAdminUsers() {
  ...
  if (error) return showAlert("Fehler beim Laden der Nutzer", "error");
  ...
  for (const user of users) {
    const sub = subs.find(s => s.user_id === user.id && s.status === "active");
    const name = user.user_metadata?.full_name || "—";
    const plan = sub ? planNames[sub.plan] : "Kein Abo";
    const status = sub ? "Aktiv" : "Inaktiv";
    const end = sub ? new Date(sub.end_date).toLocaleDateString("de-DE") : "—";

    html += `
      <tr>
        <td><strong>${name}</strong><br><small>${user.email}</small></td>
        <td>${plan}</td>
        <td><span style="color: ${sub ? 'green' : 'red'}">${status}</span></td>
        <td>${end}</td>
        <td>
          <button onclick="editUserPlan('${user.id}')" class="btn-small">Bearbeiten</button>
        </td>
      </tr>
    `;
  }
...
window.editUserPlan = async (userId) => {
  const plan = prompt("Neuer Plan (basic/premium/elite):", "basic");
  if (!plan || !["basic", "premium", "elite"].includes(plan)) return;
  ...
  if (error) showAlert("Fehler beim Aktualisieren", "error");
  else {
    showAlert("Plan aktualisiert", "success");
    loadAdminUsers();
  }
};
...
async function loadAdminPayments() {
  ...
  if (error) return showAlert("Fehler beim Laden der Zahlungen", "error");
  ...
  data.forEach(sub => {
    const name = sub.user?.user_metadata?.full_name || sub.user?.email || "Unbekannt";
    html += `
      <tr>
        <td>${name}</td>
        <td>${planNames[sub.plan]}</td>
        <td>${new Date(sub.start_date).toLocaleDateString("de-DE")}</td>
        <td>${new Date(sub.end_date).toLocaleDateString("de-DE")}</td>
        <td><span style="color: ${sub.status === 'active' ? 'green' : 'red'}">${sub.status}</span></td>
      </tr>
    `;
  });
...
async function handleContentUpload(e) {
  ...
  if (!file) return showAlert("Bitte Datei auswählen", "error");

  const ext = file.name.split(".").pop();
  const path = `${type}s/${Date.now()}_${Math.random().toString(36).substr(2, 9)}.${ext}`;
  ...
  if (fileError) return showAlert("Upload fehlgeschlagen: " + fileError.message, "error");
  ...
  const { error: dbError } = await supabase.from("content").insert({
    ...
    type,
    required_plan: form.required_plan.value,
    created_by: currentUser.id
  });

  if (dbError) {
    showAlert("Datenbankfehler: " + dbError.message, "error");
  } else {
    showAlert("Inhalt erfolgreich hochgeladen!", "success");
    form.reset();
    loadExistingContent();
    refreshMemberContent(); // Optional: Aktualisiere Mitgliederbereich
  }
}
...
async function loadExistingContent() {
  ...
  data.forEach(item => {
    html += `
      <div class="content-admin-item" style="border: 1px solid #ddd; padding: 1rem; margin: 0.5rem 0; border-radius: 8px;">
        <strong>${item.title}</strong> (${item.type}) — <em>${planNames[item.required_plan]}</em>
        <br><small>Hochgeladen: ${new Date(item.created_at).toLocaleDateString("de-DE")}</small>
        <div style="margin-top: 0.5rem;">
          <button onclick="deleteContent('${item.id}')" class="btn-small btn-danger">Löschen</button>
        </div>
      </div>
    `;
  });

  document.getElementById("existingContentList").innerHTML = html || "<p>Noch keine Inhalte.</p>";
}
...
window.deleteContent = async (id) => {
  ...
  if (error) showAlert("Löschen fehlgeschlagen", "error");
  else {
    showAlert("Inhalt gelöscht", "success");
    loadExistingContent();
    refreshMemberContent();
  }
};

// Optional: Mitgliederbereich aktualisieren
async function refreshMemberContent() {
  if (document.getElementById("membersArea").style.display === "block") {
    loadContent();
  }
}
